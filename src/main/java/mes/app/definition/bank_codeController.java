...
public class bank_codeController {
    ...
    public AjaxResult saveBankCode(
            ...
            @RequestParam("popbill_agency_code") String popbillAgencyCode,
            @RequestParam("eval_agency_code") String evalAgencyCode,
            Authentication auth
    ) {
        AjaxResult result = new AjaxResult();
        ...
        bankCode.setDescription(description);
        bankCode.setPopbillAgencyCode(popbillAgencyCode);
        bankCode.setEvalAgencyCode(evalAgencyCode);
        bankCode.set_audit(user); // 감사를 위한 사용자 기록
