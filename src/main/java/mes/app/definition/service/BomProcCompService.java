  ...
  public List<Map<String, Object>> getBomProcCompList(String routing_name, String spjangcd) {
    ...
    String sql = """
        SELECT
         ...
         LEFT JOIN process       p   ON p.id   = bpc."Process_id"
         LEFT JOIN routing       r   ON r.id   = bpc."Routing_id"
         LEFT JOIN bom           b   ON b.id   = bpc."BOM_id"
         LEFT JOIN material     mat ON mat.id = bpc."Product_id"
         LEFT JOIN material     mat2 ON mat2.id = bpc."Material_id"
         LEFT JOIN mat_grp mg   ON mg.id  = mat."MaterialGroup_id"
         LEFT JOIN mat_grp mg2   ON mg2.id  = mat2."MaterialGroup_id"
